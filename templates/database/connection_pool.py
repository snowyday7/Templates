...
class PoolMonitor:
    ...
    def record_query(self, duration: float):
        ...
        with self.lock:
            ...
            self.stats['avg_query_time'] = (total_time + duration) / self.stats['total_queries']
...
class ConnectionPoolManager:
    ...
    def create_pool(
        ...
        with self.lock:
            ...
            self.logger.info(f"Created connection pool '{name}' with {config.pool_size} connections")
            
            return engine
    
    def _register_events(self, engine: Engine, monitor: PoolMonitor, config: PoolConfig):
        """注册事件监听器"""
...
class DatabaseConnectionManager:
    ...
    def get_read_session(self):
        """获取读会话（优先使用读副本）"""
        pool_name = 'read_replica' if 'read_replica' in self.pool_manager.pools else self.default_pool
