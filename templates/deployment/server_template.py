...
"""
    ...
    def _generate_nginx_vhost(
        self, vhost: VirtualHostConfig, global_config: ServerConfig
    ) -> str:
        """生成Nginx虚拟主机配置"""
        vhost_config = f"""
...
"""
        ...
        # 基本认证
        if (
            global_config.security.basic_auth_enabled
            and global_config.security.basic_auth_file
        ):
...
"""
            ...
            for header_name, header_value in vhost.proxy_headers.items():
                vhost_config += (
                    f"            proxy_set_header {header_name} {header_value};\n"
                )
...
"""
            ...
            if vhost.ssl.hsts_enabled:
                ...
                if vhost.ssl.hsts_include_subdomains:
                    hsts_header += "; includeSubDomains"
                vhost_config += (
                    f'    Header always set Strict-Transport-Security "{hsts_header}"\n'
                )

        vhost_config += "</VirtualHost>\n"
        return vhost_config

    def generate_docker_compose_with_nginx(
        self,
        app_name: str,
        app_image: str,
        domain: str,
...
def create_reverse_proxy_config(
    ...
    return ServerConfig(server_type=ServerType.NGINX, virtual_hosts=[vhost])
