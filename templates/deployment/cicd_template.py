...
class CICDGenerator:
    ...
    def generate_github_actions(self, config: PipelineConfig) -> Dict[str, Any]:
        ...
        workflow = {
            "name": config.name,
            "on": self._build_github_triggers(config)
        }
    ...
    def generate_gitlab_ci(self, config: PipelineConfig) -> Dict[str, Any]:
        ...
        for job in config.jobs:
            ...
            job_config = {
                "stage": job_name,
                "image": job.runs_on if job.runs_on != "ubuntu-latest" else "ubuntu:latest",
...
"""
    ...
    def generate_build_pipeline(self, app_name: str, 
                               docker_config: DockerConfig,
                               test_config: TestConfig,
                               deploy_configs: List[DeploymentConfig]) -> PipelineConfig:
        """生成标准构建流水线"""
        jobs = []
        ...
        if test_config.enabled:
            ...
            if test_config.linting:
                test_steps.append(StepConfig(
                    name="Lint code",
                    command=test_config.lint_command
                ))
            
            if test_config.type_checking:
                test_steps.append(StepConfig(
                    name="Type check",
                    command=test_config.type_check_command
                ))
            
            if test_config.security_scan:
                test_steps.append(StepConfig(
                    name="Security scan",
                    command=test_config.security_scan_command
                ))
            
            if test_config.unit_tests:
                test_steps.append(StepConfig(
                    name="Run unit tests",
                    command=test_config.unit_test_command
                ))
        ...
        for deploy_config in deploy_configs:
            ...
            if deploy_config.docker:
                deploy_steps.append(StepConfig(
    ...
    def save_github_workflow(self, config: PipelineConfig, filename: Optional[str] = None) -> None:
        """保存GitHub Actions工作流"""
        if not filename:
    ...
    def save_gitlab_ci(self, config: PipelineConfig, filename: str = ".gitlab-ci.yml") -> None:
        """保存GitLab CI配置"""
    ...
    def save_jenkins_pipeline(self, config: PipelineConfig, filename: str = "Jenkinsfile") -> None:
...
def generate_build_pipeline(app_name: str, docker_config: DockerConfig,
                           test_config: TestConfig,
                           deploy_configs: List[DeploymentConfig]) -> PipelineConfig:
    """生成标准构建流水线"""
